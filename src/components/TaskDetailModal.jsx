 ...
 * Maintains original wireframe styling while integrating MongoDB field mapping.
 */
export default function TaskDetailModal({
  ...
  // Sync state when a new task is selected or modal opens
  useEffect(() => {
    if (task) {
      setEditedTask({
        ...task,
        // Ensure tags are represented as a string during editing for the input field
   ...
   * DB uses 'ownerID' (array). Map to ownerEmails for chip display.
   */
  const ownerEmails = Array.isArray(task.ownerID) ? task.ownerID : [];
  ...
  const handleSave = () => {
    ...
    const submission = {
      ...
      tags: typeof editedTask.tags === 'string'
          ? editedTask.tags.split(',').map(t => t.trim()).filter(t => t !== "")
          : editedTask.tags
    };
      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
        ...
        <div className="relative bg-white w-full max-w-lg max-h-[90vh] rounded-3xl shadow-2xl border border-slate-200 flex flex-col overflow-hidden animate-in fade-in zoom-in duration-200">
                  ...
                  <div className="flex items-center gap-2">
                    {showConfirmDelete ? (
                        <div className="flex items-center gap-2 bg-red-50 px-3 py-1.5 rounded-lg border border-red-100 animate-in slide-in-from-right-2 duration-200">
                          <button
                              onClick={() => onDelete(task._id)} // Uses MongoDB _id
          ...
          <div className="flex-1 overflow-y-auto p-8 space-y-6 custom-scrollbar">
            ...
            <div className="space-y-1.5">
              ...
              <div className="relative flex items-center">
                ...
                {isEditing ? (
                    ...
                    <p className="w-full pl-10 pr-4 py-3 font-bold text-slate-800 text-lg leading-snug">{task.label}</p>
                )}
              </div>
            </div>

            {/* Multi-Owner Display (mapped to DB 'ownerID') */}
            <div className="space-y-1.5">
              ...
              <div className="relative flex items-start">
                ...
                {isEditing ? (
                    <input
                        ...
                        value={Array.isArray(editedTask.ownerID) ? editedTask.ownerID.join(', ') : editedTask.ownerID}
                        onChange={(e) => setEditedTask({...editedTask, ownerID: e.target.value.split(',').map(m => m.trim())})}
            ...
            {/* Points & Stage Grid (mapped to DB 'phase') */}
              ...
              <div className="space-y-1.5">
                ...
                {isEditing ? (
                    ...
                    <div className="px-4 py-3 bg-blue-50/50 border border-blue-100 rounded-xl font-black text-blue-600 text-[11px] uppercase tracking-wider text-center">
                      {stageLabels[task.phase] || task.phase}
                    </div>
                )}
              </div>
            </div>
