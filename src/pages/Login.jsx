...
export default function Login() {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      if (response.ok) {
        localStorage.setItem('authentication', data.access_token);
        navigate('/dashboard');
      } else {
      ...
      <div className="flex h-screen bg-white">
        <div className="hidden lg:flex w-1/2 bg-slate-900 relative items-center justify-center overflow-hidden">
          ...
          <div className="relative z-10 text-center px-12">
            ...
            <h1 className="text-5xl font-black text-white tracking-tighter mb-4">ARGO</h1>
        ...
        <div className="w-full lg:w-1/2 flex items-center justify-center p-8">
          <div className="w-full max-w-md">
            <div className="mb-10 text-center lg:text-left">
              ...
              <p className="text-slate-500 font-bold mt-2 uppercase text-xs tracking-widest">Please enter your credentials</p>
            </div>
            ...
            <form onSubmit={handleLogin} className="space-y-6">
              ...
              <div className="space-y-6">
                ...
                <button
                    type="submit"
                    disabled={isLoading}
                    className={`w-full bg-slate-900 text-white py-4 rounded-2xl font-black uppercase tracking-[0.2em] shadow-xl shadow-slate-200 transition-all flex items-center justify-center gap-2 ${
                        isLoading ? 'opacity-70 cursor-not-allowed' : 'hover:bg-blue-600 hover:-translate-y-1 active:scale-[0.98]'
                    }`}
                >
                  {isLoading ? 'Verifying...' : 'Access System'} <ArrowRight size={20} />
                </button>
              </div>
            </form>
